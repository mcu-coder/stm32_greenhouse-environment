...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagbump)
    {
        bitFieldBitLen += bump_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagbump)
    {
        info->event[info->num] = EVENT_bump;
        info->num++;
        dataPoints->valuebump = gizVarlenDecompressionValue(bitFieldOffset,bump_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += bump_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaggz_max)
    {
        info->event[info->num] = EVENT_gz_max;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuegz_max,(uint8_t *)&issuedData[byteFieldOffset],gz_max_LEN);
        byteFieldOffset += gz_max_LEN;
        dataPoints->valuegz_max = gizX2Y(gz_max_RATIO,  gz_max_ADDITION, elongateP0FormTmp.devStatus.valuegz_max); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaggz_min)
    {
        info->event[info->num] = EVENT_gz_min;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuegz_min,(uint8_t *)&issuedData[byteFieldOffset],gz_min_LEN);
        byteFieldOffset += gz_min_LEN;
        dataPoints->valuegz_min = gizX2Y(gz_min_RATIO,  gz_min_ADDITION, elongateP0FormTmp.devStatus.valuegz_min); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwd)
    {
        info->event[info->num] = EVENT_wd;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuewd,(uint8_t *)&issuedData[byteFieldOffset],wd_LEN);
        byteFieldOffset += wd_LEN;
        dataPoints->valuewd = gizX2Y(wd_RATIO,  wd_ADDITION, elongateP0FormTmp.devStatus.valuewd); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtrsd)
    {
        info->event[info->num] = EVENT_trsd;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetrsd,(uint8_t *)&issuedData[byteFieldOffset],trsd_LEN);
        byteFieldOffset += trsd_LEN;
        dataPoints->valuetrsd = gizX2Y(trsd_RATIO,  trsd_ADDITION, elongateP0FormTmp.devStatus.valuetrsd); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuebump != cur->valuebump)
    {
        GIZWITS_LOG("valuebump Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagbump = 1;
        ret = 1;
    }
    ...
    if(last->valuegz_max != cur->valuegz_max)
    {
        GIZWITS_LOG("valuegz_max Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaggz_max = 1;
        ret = 1;
    }
    if(last->valuegz_min != cur->valuegz_min)
    {
        GIZWITS_LOG("valuegz_min Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaggz_min = 1;
        ret = 1;
    }
    if(last->valuewd != cur->valuewd)
    {
        GIZWITS_LOG("valuewd Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwd = 1;
        ret = 1;
    }
    if(last->valuetrsd != cur->valuetrsd)
    {
        GIZWITS_LOG("valuetrsd Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtrsd = 1;
        ret = 1;
    }
    ...
    if(last->valuets != cur->valuets)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuets Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagts = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagbump)
    {
        gizVarlenCompressValue(bitFieldOffset,bump_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuebump);
        bitFieldOffset += bump_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagts)
    {
        devStatusTmp.valuets = gizY2X(ts_RATIO,  ts_ADDITION, dataPoints->valuets); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuets,ts_LEN);
        byteFieldOffset += ts_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaggz_max)
    {
        devStatusTmp.valuegz_max = gizY2X(gz_max_RATIO,  gz_max_ADDITION, dataPoints->valuegz_max); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuegz_max,gz_max_LEN);
        byteFieldOffset += gz_max_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaggz_min)
    {
        devStatusTmp.valuegz_min = gizY2X(gz_min_RATIO,  gz_min_ADDITION, dataPoints->valuegz_min); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuegz_min,gz_min_LEN);
        byteFieldOffset += gz_min_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagwd)
    {
        devStatusTmp.valuewd = gizY2X(wd_RATIO,  wd_ADDITION, dataPoints->valuewd); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewd,wd_LEN);
        byteFieldOffset += wd_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtrsd)
    {
        devStatusTmp.valuetrsd = gizY2X(trsd_RATIO,  trsd_ADDITION, dataPoints->valuetrsd); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetrsd,trsd_LEN);
        byteFieldOffset += trsd_LEN;
    }
